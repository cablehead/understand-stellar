...
used. The CSS is the demonstration.
...
HTML tags are the blocks that come in the box: `p`, `a`, `h1` through `h6`,
`code`, `ul`, `li`, `table`, and the rest. Style them once, well, with no
...
z-index demo. Build one as a server component, a Nushell `def` that composes raw
tags into the new block. Give the def one class, on its root, and style
...
before writing the `def` and its class. If raw tags and the blocks already built
can express the thing, they must. A class that dresses one element, in one
place, named for the spot rather than for a block, is not a block. It is a
literal with a name. Fold it back into the tags.
