...
(async () => {
  ...
  const restStatic = await page.evaluate(async () => {
    ...
    return { animating: getComputedStyle(box).animationName !== 'none', moved: a !== getComputedStyle(box).transform };
  });
  ...
  const durPlay = await page.evaluate(async () => {
    ...
    return { before, pulsed: max - min > 10, peak: Math.round(max), clearedAfter: after < 3 };
  });
  ...
  const copy = await page.evaluate(async () => {
    ...
    return { toast: toast ? toast.textContent : '', shown: toast && toast.classList.contains('show') };
  });

  if (shot) {
    await page.evaluate(() => {
      const h = [...document.querySelectorAll('h3')].find((e) => /Duration/i.test(e.textContent));
  ...
  console.log(JSON.stringify({ url, jsErrors: errors, restStatic, tfPlay, durPlay, easePlay, copy, shot }, null, 1));
  await browser.close();
})();
