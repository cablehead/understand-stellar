...
(async () => {
  ...
  const out = await page.evaluate(() => {
    ...
    return {
      ...
      v002_progress: test(
        "opacity: clamp(0, calc((100vw - 320px) / (1440px - 320px)), 1);",
        "opacity",
      ),
      // 0.0.2 full slow token. expect ~0.343s.
      v002_slow_duration: test(
        "transition-duration: calc(calc(0.28 + 0.08 * clamp(0, calc((100vw - 320px) / (1440px - 320px)), 1)) * 1s);",
        "transitionDuration",
      ),
      // 0.0.1 broken token for contrast. expect 0s.
      v001_slow_duration: test(
        "transition-duration: calc(calc(0.28 + 0.08 * clamp(0, calc((100vw - 320px) * 0.000893), 1)) * 1s);",
        "transitionDuration",
      ),
    };
  });
